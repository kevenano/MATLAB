function [E,Et]=main(XS,YS,yeta,turns,XST,YST)
%MAIN 训练主函数
%   XS: 所有样本输入值
%   YS: 所有样本输出值
%   yeta: 学习率
...
E = zeros(1, turns);       % 训练集累计误差
Et = zeros(1, turns);      % 测试集累计误差
for turn = 1:turns
    for k = 1:m
        X = XS{k};                          % 当前样本输入值
        Y = YS{k};                          % 当前样本输出值
    ...
    E(turn) = 1/m*Ek*(Ek.');                % 计算训练集的累计误差
    v_t = v;
    w_t = w;
    theta_t = theta;
    n_t = n;
    [Et(turn),~] = testCheck(XST,YST,v_t,w_t,theta_t,n_t);  % 计算测试集的累计误差
    sprintf('Turns %d, E:%.10f, Et:%.10f',turn,E(turn),Et(turn))
end
end
